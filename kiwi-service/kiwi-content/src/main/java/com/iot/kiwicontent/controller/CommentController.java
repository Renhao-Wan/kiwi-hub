...
public class CommentController {
    ...
    public Result<Object> deleteComment(@Parameter(description = "评论ID", required = true) @PathVariable("commentId") Long commentId) {
        Long userId = UserContext.getUserId();
    ...
    public Result<PageResult<CommentVO>> getRootComments(
            @Parameter(description = "文章ID", required = true) @RequestParam("articleId") Long articleId,
            @Parameter(description = "页码") @RequestParam(defaultValue = "1") Integer pageNum,
            @Parameter(description = "每页数量") @RequestParam(defaultValue = "10") Integer pageSize) {
        return Result.success(commentService.getRootComments(articleId, pageNum, pageSize));
    }

    @Operation(summary = "获取楼中楼回复列表", description = "游标分页，parentId=rootId 查二级评论（携带replyCount），parentId=二级评论id 查三级回复")
    @GetMapping("/replies")
